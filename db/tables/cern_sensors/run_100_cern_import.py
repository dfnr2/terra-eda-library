#!/usr/bin/env python3
# db/tables/cern_sensors/run_100_cern_import.py
"""Import CERN 'Sensors' -> cern_sensors_generated_100_cern_import.sql.

No type-specific tail: 'Sensors' is a grab-bag category (temperature, current,
Hall, pressure, humidity, gas, optical, accelerometer/IMU, SiPM, image sensors,
…) whose CERN symbols are device-named and which has no deterministic sensor
category dimension ('Component Type' uniformly 'Standard', no 'Family' column).
Per the port-cern-library skill, "no tail" is the correct outcome here.
"""
...
CERN_TABLE = "Sensors"
OUTPUT_FILE = "cern_sensors_generated_100_cern_import.sql"
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_sensors ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'sensor');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_sensors parts")
