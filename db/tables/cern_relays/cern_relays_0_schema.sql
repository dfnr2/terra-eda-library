-- db/tables/cern_relays/cern_relays_0_schema.sql
-- Terra EDA Library - cern_relays table schema (CERN 'Relays').
-- Core fields match the go-forward core; tier defaults to 5 per TIER_TAG_SPEC.org.

CREATE TABLE cern_relays (
    ...
    -- Relays-specific tail
    relay_kind TEXT,     -- 'Relay' | 'Socket' (from the CERN symbol-name prefix REL_/RELS_/SOCKET)
    coil_voltage TEXT    -- coil voltage from the CERN Value column (e.g. '24VDC', '230VAC'); blank for sockets
);
