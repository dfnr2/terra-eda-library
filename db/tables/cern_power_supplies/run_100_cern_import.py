#!/usr/bin/env python3
# db/tables/cern_power_supplies/run_100_cern_import.py
"""Import CERN 'Power Supplies' -> cern_power_supplies_generated_100_cern_import.sql."""
...
CERN_TABLE = "Power Supplies"
OUTPUT_FILE = "cern_power_supplies_generated_100_cern_import.sql"
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_power_supplies ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'power-supply');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_power_supplies parts")
