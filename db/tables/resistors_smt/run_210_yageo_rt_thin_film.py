...
def get_enabled_tolerances() -> List[str]:
    ...
    Returns list of enabled tolerance strings (e.g., ["0.1%", "0.5%", "1%"]).
    """
...
def generate_resistors() -> str:
    ...
    for spec in specs_to_generate:
        ...
        for resistance_ohms in values_to_generate:
            ...
            # All thin film parts → tier 3 (specialty/precision)
            part_tier = 3
            ...
            sql_line = SQL_TEMPLATES["insert"].format(
                ...
                tier=part_tier,
                keywords='passive,precision',
                tolerance=spec["tolerance"],
                power_rating=spec["power"],
                temp_coeff=spec["tempco"],
                voltage_rating=spec["working_voltage"],
                composition=COMPOSITION,
                temp_operating_min=parse_temp_range(spec["temp_operating"])[0],
                temp_operating_max=parse_temp_range(spec["temp_operating"])[1],
            ...
            sql_lines.append(SQL_TEMPLATES["tag_insert"].format(
                unique_id=unique_id, tag='precision'))
            total_parts += 1
