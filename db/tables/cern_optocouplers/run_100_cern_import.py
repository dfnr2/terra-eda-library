#!/usr/bin/env python3
# db/tables/cern_optocouplers/run_100_cern_import.py
"""Import CERN 'Optocouplers' -> cern_optocouplers_generated_100_cern_import.sql."""
...
CERN_TABLE = "Optocouplers"
OUTPUT_FILE = "cern_optocouplers_generated_100_cern_import.sql"
...
INSERT_COLS = [
    ...
    "optocoupler_type", "channels",
]

# Trailing descriptors in a symbol name that are not part of the base type.
_TYPE_CUT = re.compile(r"(?:\s+x\d+|\s*Type|\s+\[alt\]|\s+\+|\s+EP\b|_a\b)")
_CHANNELS = re.compile(r"\bx(\d)\b")
# The extracted prefix is a real type only if it names one; otherwise the symbol
# is a bare device part number (e.g. "VOM1271") -> leave blank.
_TYPE_KW = re.compile(r"OPTOCOUPLER|PHOTOMOS|PHOTO|RELAY|OPTO|COUPLER|SOLID\s*STATE", re.I)
...
def opto_fields_from_symbol(libsymbol: str) -> tuple[str, str]:
    """(optocoupler_type, channels) from the CERN symbol name.

    e.g. 'Optocoupler x2 Type2' -> ('Optocoupler', '2');
    'PhotoMOS Relay x1 Type5_a' -> ('PhotoMOS Relay', '1').
    """
    ...
    return (base, channels) if _TYPE_KW.search(base) else ("", channels)
...
def map_row(row: dict) -> dict:
    ...
    opto_type, channels = opto_fields_from_symbol(clean(row.get("LibSymbol")))
    return {
        "unique_id": f"{mfr}-{mpn}",
        "part_locator": pnn,
        "mpn": mpn,
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_optocouplers ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'optocoupler');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_optocouplers parts")
