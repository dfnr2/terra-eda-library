#!/usr/bin/env python3
# db/tables/cern_transistors/run_100_cern_import.py
"""Import CERN 'Transistors' -> cern_transistors_generated_100_cern_import.sql."""
...
CERN_TABLE = "Transistors"
OUTPUT_FILE = "cern_transistors_generated_100_cern_import.sql"
...
# A symbol-name token that designates a pin (e.g. 1G, B1, 3D) — marks the end of
# the type prefix in names like "N-MOSFET-D 1G 2S 3D" / "NPN B1 E2 C3".
_PIN_TOKEN = re.compile(r"^(?:\d*[A-Z]\d+|\d+[A-Z])$")
# The extracted prefix is only a real transistor type if it names one; otherwise
# the symbol is just a device part number (e.g. "IXGR32N170H1") -> leave blank.
_TYPE_KW = re.compile(
    r"MOSFET|NPN|PNP|JFET|IGBT|SIPMOS|DARLINGTON|BJT|SCR|TRIAC|THYRISTOR|UJT|"
    r"HEMT|GAN|SIC|\bFET\b", re.I)
...
def transistor_type_from_symbol(libsymbol: str) -> str:
    name = libsymbol.split(":", 1)[1] if ":" in libsymbol else libsymbol
    prefix = []
    for tok in name.split():
        if _PIN_TOKEN.match(tok):
            break
        prefix.append(tok)
    t = " ".join(prefix).strip()
    return t if _TYPE_KW.search(t) else ""
...
def map_row(row: dict) -> dict:
    ...
    return {
        ...
        "transistor_type": transistor_type_from_symbol(clean(row.get("LibSymbol"))),
    }
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_transistors ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'transistor');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_transistors parts")
