#!/usr/bin/env python3
# db/tables/cern_transformers/run_100_cern_import.py
"""Import CERN 'Transformers' -> cern_transformers_generated_100_cern_import.sql.

No type-specific tail: 'Transformers' is a grab-bag category (mains power, signal/
audio, pulse, RF balun, gate-drive, current-sense, common-mode choke, …) whose
CERN symbols are bespoke vendor-part-named (XFMR_<VENDOR>_<PART>) or generic
winding-topology symbols ('Transformer 2xPrim 2xSec Type1'), with no deterministic
transformer-category dimension ('Component Type'/'Component Kind' uniformly
'Standard', no 'Family' column). Parsing turns-ratio/inductance from free text is
out of scope. Per the port-cern-library skill, "no tail" is the correct outcome.
"""
...
CERN_TABLE = "Transformers"
OUTPUT_FILE = "cern_transformers_generated_100_cern_import.sql"
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_transformers ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'transformer');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_transformers parts")
