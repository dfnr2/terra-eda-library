#!/usr/bin/env python3
# db/tables/cern_switches/run_100_cern_import.py
"""Import CERN 'Switches' -> cern_switches_generated_100_cern_import.sql."""
...
CERN_TABLE = "Switches"
OUTPUT_FILE = "cern_switches_generated_100_cern_import.sql"
...
def switch_type_from_symbol(libsymbol: str) -> str:
    """Switch sub-type from the CERN symbol-name prefix.

    The CERN 'Switches' library names every symbol with a deterministic
    leading token: 'PB …' push-buttons, 'SW … DIP Switch' DIP arrays, 'SW
    Rotary …' rotary/coded switches, 'Knob'/'KNB' knobs, 'SMD Solder Jumper'
    solder jumpers, 'Tilt Switch …' tilt switches, and the remaining 'SW …'
    are toggle/slide/rocker generic switches. Classifies all 226 rows; the
    Family column is a grab-bag ('Switch' mixes all of these) so the symbol
    prefix is the only clean type dimension.
    """
    name = libsymbol.split(":", 1)[1] if ":" in libsymbol else libsymbol
    if "DIP Switch" in name:
        return "dip"
    if name.startswith("PB"):
        return "push-button"
    if name.startswith("SW Rotary"):
        return "rotary"
    if name.startswith("Knob") or name.startswith("KNB"):
        return "knob"
    if "Solder Jumper" in name:
        return "jumper"
    if "Tilt Switch" in name:
        return "tilt"
...
def map_row(row: dict) -> dict:
    ...
    return {
        ...
        "switch_type": switch_type_from_symbol(clean(row.get("LibSymbol"))),
    }
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_switches ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'switch');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_switches parts")
