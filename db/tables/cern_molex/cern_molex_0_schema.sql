-- db/tables/cern_molex/cern_molex_0_schema.sql
-- Terra EDA Library - cern_molex table schema (CERN 'MOLEX' connectors).
-- Core fields match the go-forward core; tier defaults to 5 per TIER_TAG_SPEC.org.
-- Connectors share the "Connectors" symbol lib; type is captured by the symbol +
-- pin_count, so no type-specific tail.

CREATE TABLE cern_molex (
