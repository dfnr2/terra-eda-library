#!/usr/bin/env python3
# db/tables/cern_molex/run_100_cern_import.py
"""Import CERN 'MOLEX' -> cern_molex_generated_100_cern_import.sql."""
...
CERN_TABLE = "MOLEX"
OUTPUT_FILE = "cern_molex_generated_100_cern_import.sql"
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_molex ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'connector');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_molex parts")
