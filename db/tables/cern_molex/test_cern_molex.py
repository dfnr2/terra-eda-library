...
DB = ROOT / "db/cern_molex.db"


def _con():
    # CERN.sqlite is resolved by tools.cern_source (vendored clone or $CERN_SQLITE).
    subprocess.run(["make", "cern_molex-build"], cwd=ROOT, check=True, capture_output=True)
    return sqlite3.connect(DB)


def test_row_count_is_503():
    con = _con()
    assert con.execute("SELECT COUNT(*) FROM cern_molex").fetchone()[0] == 503


def test_no_duplicate_unique_ids():
    con = _con()
    n, d = con.execute(
        "SELECT COUNT(*), COUNT(DISTINCT unique_id) FROM cern_molex").fetchone()
    assert n == d
...
def test_known_part_mapping():
    ...
    r = con.execute("SELECT * FROM cern_molex WHERE mpn='105133-0011'").fetchone()
    assert r["manufacturer"] == "MOLEX"
    assert r["unique_id"] == "MOLEX-105133-0011"
    assert r["pin_count"] == "8"
    assert r["kicad_symbol"] == "cern-connectors:Connector USB 5 Pin + 1Shield"
    assert r["kicad_footprint"] == "cern-molex-smd:MOLEX_105133-0011"
...
def test_all_symbol_nicks_registered():
    ...
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_symbol FROM cern_molex WHERE kicad_symbol != ''"):
        nick = val.split(":", 1)[0]
        assert f'(name "{nick}")' in sym, f"unregistered symbol nick: {nick}"
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_footprint FROM cern_molex WHERE kicad_footprint LIKE '%:%'"):
        nick = val.split(":", 1)[0]
...
def test_all_symbol_items_resolve():
    ...
    missing = []
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_symbol FROM cern_molex WHERE kicad_symbol LIKE '%:%'"):
        nick, name = val.split(":", 1)
