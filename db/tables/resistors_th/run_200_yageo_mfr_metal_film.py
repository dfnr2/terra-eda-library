...
# ======================== CONFIGURATION ========================
# Tolerance code -> label, the E-series it's stocked in, and enable flag.
# Convention: precision grades (0.5%, 1%) come in E96+E24; wide grades (2%, 5%)
# in E24 only -- so we don't synthesize 5% parts at E96-only values.
TOLERANCES = [
    {"code": "D", "label": "±0.5%", "enable": "yes", "eseries": ["E24", "E96"]},
    {"code": "F", "label": "±1%",  "enable": "yes", "eseries": ["E24", "E96"]},
    {"code": "G", "label": "±2%",  "enable": "no",  "eseries": ["E24"]},
    {"code": "J", "label": "±5%",  "enable": "yes", "eseries": ["E24"]},
]
...
def build() -> str:
    ...
    # One orderable part per distinct manufacturer value code, per tolerance's
    # E-series: distinct E24/E96 floats can share an IEC code (E24 1.1 vs E96
    # 1.10) and the MPN is keyed on that code, so dedupe by code, not by float.
    def codes_for(eseries):
        cto = {}
        for v in sorted({x for s in eseries for x in e_series_values(series_map[s])}):
            cto.setdefault(encode_resistance(v, "iec"), v)
        return cto
    tol_codes = {t["code"]: codes_for(t["eseries"]) for t in tols}

    created = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    script = Path(__file__).name
    tol_str = ", ".join(f"{t['label']} [{'+'.join(t['eseries'])}]" for t in tols)

    lines = [
        f"-- {MANUFACTURER} {SERIES} Series {COMPOSITION} axial through-hole resistors",
        f"-- Tolerances: {tol_str}; TCR {TCR_LABEL}",
    ...
    for spec in POWER_SPECS:
        ...
        for tol in tols:
            for value_iec, ohm in tol_codes[tol["code"]].items():
