#!/usr/bin/env python3
# db/tables/cern_fuses/run_100_cern_import.py
"""Import CERN 'Fuses' -> cern_fuses_generated_100_cern_import.sql."""
...
CERN_TABLE = "Fuses"
OUTPUT_FILE = "cern_fuses_generated_100_cern_import.sql"
...
# The CERN 'Family' column is the deterministic type dimension for fuses (filled
# 315/315). Normalize the one odd plural ('Fuses Resettable') and otherwise copy
# the family verbatim. Every value here is a real fuse-domain part type.
FAMILY_TO_KIND = {
    "Fuse": "Fuse",
    "Fuses Resettable": "Fuse Resettable",
    "Fuse Holder": "Fuse Holder",
    "Fuse Holder Cover": "Fuse Holder Cover",
    "Fuse Clip": "Fuse Clip",
    "Fuse & Holder": "Fuse & Holder",
...
def fuse_kind_from_family(family: str) -> str:
    """Deterministic fuse type from the CERN 'Family' column (always filled)."""
    return FAMILY_TO_KIND.get(clean(family), clean(family))
...
def map_row(row: dict) -> dict:
    ...
    # CERN puts the current/voltage rating in Value (e.g. '1A-32V', '500mA-63V');
    # the blank-Value rows are holders/clips/covers, which carry no rating.
    rating = clean(row.get("Value"))
    return {
        "unique_id": f"{mfr}-{mpn}",
        "part_locator": pnn,
        "mpn": mpn,
        ...
        "value": rating or clean(row.get("Part Description")),
        ...
        "fuse_kind": fuse_kind_from_family(row.get("Family")),
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_fuses ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'fuse');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_fuses parts")
