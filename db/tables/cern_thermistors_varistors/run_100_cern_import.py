#!/usr/bin/env python3
# db/tables/cern_thermistors_varistors/run_100_cern_import.py
"""Import CERN 'Thermistors And Varistors' -> cern_thermistors_varistors_generated_100_cern_import.sql."""
...
CERN_TABLE = "Thermistors And Varistors"
OUTPUT_FILE = "cern_thermistors_varistors_generated_100_cern_import.sql"
...
# The CERN 'LibSymbol' name is the deterministic type dimension here (filled
# 53/53): the table cleanly splits into thermistors (NTC / PTC) vs varistors,
# plus one TCO (thermal cut-off resistor+fuse) symbol. Map the symbol item name
# to a normalized device_type.
SYMBOL_TO_DEVICE = {
    "Thermistor NTC": "NTC",
    "Thermistor PTC": "PTC",
    "Thermistor PTC - polarized": "PTC",
    "Varistor": "Varistor",
    "TCO Resistor + Fuse": "TCO",
}
...
def device_type_from_symbol(lib_symbol: str) -> str:
    """Deterministic device type from the CERN 'LibSymbol' item name."""
    name = clean(lib_symbol).split(":", 1)[-1]
    return SYMBOL_TO_DEVICE.get(name, "")
...
def map_row(row: dict) -> dict:
    ...
    return {
        ...
        "value": val or clean(row.get("Part Description")),
        ...
        "device_type": device_type_from_symbol(row.get("LibSymbol")),
        "voltage": clean(row.get("Voltage")),
    }
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_thermistors_varistors ({cols_sql}) "
            f"VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'thermistor_varistor');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_thermistors_varistors parts")
