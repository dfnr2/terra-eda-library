#!/usr/bin/env python3
# db/tables/cern_logic/run_100_cern_import.py
"""Import CERN 'Logic' -> cern_logic_generated_100_cern_import.sql."""
...
CERN_TABLE = "Logic"
OUTPUT_FILE = "cern_logic_generated_100_cern_import.sql"
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_logic ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_logic parts")
