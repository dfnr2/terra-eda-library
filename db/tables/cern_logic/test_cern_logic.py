...
DB = ROOT / "db/cern_logic.db"


def _con():
    # CERN.sqlite is resolved by tools.cern_source (vendored clone or $CERN_SQLITE).
    subprocess.run(["make", "cern_logic-build"], cwd=ROOT, check=True, capture_output=True)
    return sqlite3.connect(DB)


def test_row_count_is_761():
    con = _con()
    assert con.execute("SELECT COUNT(*) FROM cern_logic").fetchone()[0] == 761


def test_no_duplicate_unique_ids():
    con = _con()
    n, d = con.execute(
        "SELECT COUNT(*), COUNT(DISTINCT unique_id) FROM cern_logic").fetchone()
    assert n == d
...
def test_known_part_mapping():
    ...
    r = con.execute("SELECT * FROM cern_logic WHERE mpn='MCP4728-E/UN'").fetchone()
    assert r["manufacturer"] == "MICROCHIP TECHNOLOGY"
    assert r["unique_id"] == "MICROCHIP TECHNOLOGY-MCP4728-E/UN"
    assert r["package"] == "MSOP10"
    assert r["pin_count"] == "10"
    assert r["kicad_symbol"] == "cern-logic:MCP4728"
    assert r["kicad_footprint"] == "cern-ics-and-semiconductors-smd:SOP50P490X110-10N"
...
def test_all_symbol_nicks_registered():
    ...
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_symbol FROM cern_logic WHERE kicad_symbol != ''"):
        nick = val.split(":", 1)[0]
        assert f'(name "{nick}")' in sym, f"unregistered symbol nick: {nick}"
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_footprint FROM cern_logic WHERE kicad_footprint LIKE '%:%'"):
        nick = val.split(":", 1)[0]
...
def test_all_symbol_items_resolve():
    ...
    missing = []
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_symbol FROM cern_logic WHERE kicad_symbol LIKE '%:%'"):
        nick, name = val.split(":", 1)
