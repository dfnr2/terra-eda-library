#!/usr/bin/env python3
# db/tables/cern_standard_logic/run_100_cern_import.py
"""Import CERN 'Standard Logic' -> cern_standard_logic_generated_100_cern_import.sql."""
...
CERN_TABLE = "Standard Logic"
OUTPUT_FILE = "cern_standard_logic_generated_100_cern_import.sql"
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_standard_logic ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'logic');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_standard_logic parts")
