#!/usr/bin/env python3
# db/tables/cern_batteries/run_100_cern_import.py
"""Import CERN 'Batteries' -> cern_batteries_generated_100_cern_import.sql."""
...
CERN_TABLE = "Batteries"
OUTPUT_FILE = "cern_batteries_generated_100_cern_import.sql"
...
def battery_kind_from_symbol(lib_symbol: str) -> str:
    """Deterministic cell-vs-holder split from the CERN 'LibSymbol' item name.

    The Batteries table's symbol names cleanly split into the actual battery
    ('Battery No Pin' coin/cylindrical cells, 'Battery 2 Pin' rechargeable) vs a
    holder/clip ('Battery Holder N Pin …'). 29/29 filled.
    """
    name = clean(lib_symbol).split(":", 1)[-1]
    if name.startswith("Battery Holder"):
        return "holder"
...
def map_row(row: dict) -> dict:
    ...
    return {
        ...
        "battery_kind": battery_kind_from_symbol(row.get("LibSymbol")),
        "voltage": clean(row.get("Value")),
    }
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_batteries ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'battery');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_batteries parts")
