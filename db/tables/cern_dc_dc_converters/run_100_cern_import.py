#!/usr/bin/env python3
# db/tables/cern_dc_dc_converters/run_100_cern_import.py
"""Import CERN 'DC-DC Converters' -> cern_dc_dc_converters_generated_100_cern_import.sql."""
...
CERN_TABLE = "DC-DC Converters"
OUTPUT_FILE = "cern_dc_dc_converters_generated_100_cern_import.sql"
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_dc_dc_converters ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'dcdc');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_dc_dc_converters parts")
