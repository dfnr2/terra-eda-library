#!/usr/bin/env python3
# db/tables/cern_regulators/run_100_cern_import.py
"""Import CERN 'Regulators' -> cern_regulators_generated_100_cern_import.sql."""
...
CERN_TABLE = "Regulators"
OUTPUT_FILE = "cern_regulators_generated_100_cern_import.sql"
...
def regulator_type_from_symbol(libsymbol: str) -> str:
    """Coarse type from the symbol-name prefix (deterministic, not inferred).

    CERN's generic regulator symbols start with REG (e.g. 'REG3 1IN-2GND-3OUT')
    and reference symbols with VREF/REF. Device-named symbols (a bare part
    number, e.g. 'ADR44x') get no type.
    """
    name = libsymbol.split(":", 1)[1] if ":" in libsymbol else libsymbol
    head = name.split()[0].upper() if name.split() else ""
    if head.startswith("VREF") or head.startswith("REF"):
        return "Voltage Reference"
    if head.startswith("REG"):
        return "Regulator"
    return ""
...
def map_row(row: dict) -> dict:
    ...
    return {
        ...
        "regulator_type": regulator_type_from_symbol(clean(row.get("LibSymbol"))),
    }
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_regulators ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'regulator');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_regulators parts")
