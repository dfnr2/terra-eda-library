...
DB = ROOT / "db/cern_analog_interface.db"


def _con():
    # CERN.sqlite is resolved by tools.cern_source (vendored clone or $CERN_SQLITE).
    subprocess.run(["make", "cern_analog_interface-build"], cwd=ROOT, check=True, capture_output=True)
    return sqlite3.connect(DB)


def test_row_count_is_2199():
    con = _con()
    assert con.execute("SELECT COUNT(*) FROM cern_analog_interface").fetchone()[0] == 2199


def test_no_duplicate_unique_ids():
    con = _con()
    n, d = con.execute(
        "SELECT COUNT(*), COUNT(DISTINCT unique_id) FROM cern_analog_interface").fetchone()
    assert n == d
...
def test_known_part_mapping():
    ...
    r = con.execute("SELECT * FROM cern_analog_interface WHERE mpn='NE555D'").fetchone()
    assert r["manufacturer"] == "TEXAS INSTRUMENTS"
    assert r["unique_id"] == "TEXAS INSTRUMENTS-NE555D"
    assert r["package"] == "SOIC8"
    assert r["pin_count"] == "8"
    assert r["kicad_symbol"] == "cern-analog-interface:NE555"
...
def test_all_symbol_nicks_registered():
    ...
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_symbol FROM cern_analog_interface WHERE kicad_symbol != ''"):
        nick = val.split(":", 1)[0]
        assert f'(name "{nick}")' in sym, f"unregistered symbol nick: {nick}"
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_footprint FROM cern_analog_interface WHERE kicad_footprint LIKE '%:%'"):
        nick = val.split(":", 1)[0]
...
def test_all_symbol_items_resolve():
    ...
    missing = []
    for (val,) in con.execute(
            "SELECT DISTINCT kicad_symbol FROM cern_analog_interface WHERE kicad_symbol LIKE '%:%'"):
        nick, name = val.split(":", 1)
