#!/usr/bin/env python3
# db/tables/cern_crystals_oscillators/run_100_cern_import.py
"""Import CERN 'Crystals & Oscillators' -> cern_crystals_oscillators_generated_100_cern_import.sql."""
...
CERN_TABLE = "Crystals & Oscillators"
OUTPUT_FILE = "cern_crystals_oscillators_generated_100_cern_import.sql"
...
def xtal_type_from_symbol(libsymbol: str) -> str:
    """'Quartz Crystal' | 'Oscillator' from the CERN symbol name; blank otherwise.

    Symbol names lead with the base type ('Quartz Crystal 1X 2GND 3X 4GND',
    'Oscillator 1E 2GND 3OUT 4Vs', 'OSC_CRYSTEK_CCLD-033'); bare device-named
    symbols (LTC6909, MC100EL1648DG, ROS-XXXXX, AD2S99) carry no type -> blank.
    """
    name = libsymbol.split(":", 1)[1] if ":" in libsymbol else libsymbol
    if name.startswith("Quartz Crystal"):
        return "Quartz Crystal"
    if name.startswith(("Oscillator", "OSC_")):
        return "Oscillator"
    return ""
...
def map_row(row: dict) -> dict:
    ...
    # CERN puts the nominal frequency in Value (e.g. '25MHz'); the 11 blank-Value
    # rows are silicon oscillator ICs / VCOs whose description carries the range.
    freq = clean(row.get("Value"))
    return {
        "unique_id": f"{mfr}-{mpn}",
        "part_locator": pnn,
        "mpn": mpn,
        ...
        "value": freq or clean(row.get("Part Description")),
...
def render(mapped: list[dict]) -> str:
    ...
    for m in mapped:
        ...
        lines.append(
            f"INSERT INTO cern_crystals_oscillators ({cols_sql}) VALUES ({', '.join(vals)});")
    for m in mapped:
        lines.append(
            "INSERT INTO tags (unique_id, tag) VALUES "
            f"({sqlstr(m['unique_id'])}, 'crystal-oscillator');")
    lines.append("COMMIT;")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"+ Wrote {OUTPUT_FILE}: {len(mapped)} cern_crystals_oscillators parts")
